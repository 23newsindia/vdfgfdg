...
class OC_Cache {
    ...
    const TRANSIENT_EXPIRY = 15 * MINUTE_IN_SECONDS;
     ...
     public function __construct() {
        // Existing hooks
        add_action('oc_carousel_updated', [$this, 'clear_carousel_cache']);
        add_action('oc_carousel_deleted', [$this, 'clear_carousel_cache']);
        add_action('updated_option', [$this, 'maybe_clear_all_cache']);
        register_deactivation_hook(__FILE__, [$this, 'clear_all_cache']);

        // Add these new hooks for broader cache clearing:
        add_action('save_post', [$this, 'clear_post_cache']); // Clear if post content changes
        add_action('woocommerce_settings_saved', [$this, 'clear_all_cache']); // WC settings update
        add_action('switch_theme', [$this, 'clear_all_cache']); // Theme change
        
        // Additional WooCommerce-specific hooks
        add_action('woocommerce_product_options_pricing', [$this, 'maybe_clear_product_cache']);
    ...
    public function clear_carousel_cache($carousel_id) {
        ...
        if ($carousel) {
            ...
            $base_key = $this->get_cache_key('carousel', $carousel->slug);
 ...
 */
public function maybe_clear_all_cache($option_name) {
    $critical_options = [
 ...
 */
public function get_shortcode_cache($carousel_slug) {
...
public function set_shortcode_cache($carousel_slug, $html) {
    $cache_key = '_oc_shortcode_cache_' . md5($carousel_slug);
  ...
  public function maybe_clear_product_cache() {
